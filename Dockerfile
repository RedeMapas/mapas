...
RUN CI=true pnpm install --frozen-lockfile 2>/dev/null || CI=true pnpm install
RUN pnpm run build
...
# Install runtime dependencies (including nginx and ca-certificates first to avoid TLS errors)
RUN apk add --no-cache \
  ca-certificates \
  sudo \
  bash \
...
# Install build dependencies, build PHP extensions, install PECL extensions, and cleanup in one layer
...
RUN apk add --no-cache nodejs npm && \
  npm install -g pnpm@9 sass terser uglifycss autoprefixer postcss
