...
/**
 * @property string $id id da aplicação
 * @property Slim\App $slim instância do Slim
 * @property Hooks $hooks gerenciador de hooks
 * @property EntityManager $em Doctrine Entity Manager
 * @property AuthProvider $auth provedor de autenticação
 * @property-read string $siteName nome do site
 * @property-read string $siteDescription descrição do site
 * @property Subsite $subsite Subsite atual
 ...
 * @property int|null $currentSubsiteId id do subsite atual
 ...
 * @property-read RoutesManager $routesManager gerenciador de rotas
 * @property-read string $baseUrl URL base da aplicação
 * @property-read string $assetUrl URL dos assets
 * @property-read TransportInterface $mailerTransport transporte de e-mail
 * @property-read Mailer $mailer serviço de e-mail
 * @property-read Connection $conn conexão com o banco de dados
 *
 *
 * @property ResponseInterface $response objeto de resposta HTTP
 * @property Request $request objeto de requisição HTTP
 *
 * @package MapasCulturais
 */
class App
{
    use Traits\MagicCallers, Traits\MagicGetter, Traits\MagicSetter;
     ...
     *
     * Aqui são armazenados os controladores registrados, tipos de entidade, grupos de tipos de entidade,
     * definições de metadados de entidade, definições de grupos de arquivos e definições de taxonomia.
     *
     * @var array
     */
    protected $_register = [
    ...
    protected function __construct(string $id)
    {
        ...
        $error_middleware = $this->slim->addErrorMiddleware(
     ...
     *
     * Limpa caches, filas e componentes
     *
     * @return void
     */
    function reset()
    ...
    /**
     * Retorna o registro da aplicação
     *
     * @return array
     */
    ...
    function init(array $config)
    {
        ...
        if ($config["app.offline"]) {
            ...
            if (
                php_sapi_name() !== "cli" &&
                (!is_callable($bypass_callable) || !$bypass_callable())
            ) {
        ...
        $instance = $instance->withRuleNamespace(
            "MapasCulturais\\Validators\\Rules",
        );
        $instance = $instance->withExceptionNamespace(
            "MapasCulturais\\Validators\\Exceptions",
        );
    ...
    protected function _initLogger()
    {
        ...
        if (is_string($this->config["monolog.handlers"])) {
            ...
            foreach ($handlers_config as $handler_config) {
                ...
                $level =
                    $handler_config[1] ?? $this->config["monolog.defaultLevel"];

                if ($type == "file") {
                    $handlers[] = new Handler\StreamHandler(
                        $this->config["monolog.logsDir"] . "app.log",
                        "DEBUG",
                    );
                    ...
                    $api_key =
                        (string) $this->config["monolog.telegram.apiKey"] ??
                        false;
                    $channel_id =
                        (string) $this->config["monolog.telegram.channelId"] ??
                        false;

                    if ($api_key && $channel_id) {
                        $handler = new Handler\TelegramBotHandler(
                            $api_key,
                            $channel_id,
                            $level,
                            parseMode: "Markdown",
    ...
    protected function _initAutoloader()
    {
        ...
        foreach (scandir(THEMES_PATH) as $ff) {
            if ($ff != "." && $ff != "..") {
                ...
                if (
                    is_dir($theme_folder) &&
                    file_exists($theme_folder . "/Theme.php")
                ) {
                    ...
                    if (
                        ...
                        $namespace = $matches[1];
                        if (
                            !array_key_exists($namespace, $config["namespaces"])
                        ) {
        ...
        spl_autoload_register(function ($class) use (&$config) {
            ...
            $namespaces[
                "MapasCulturais\\DoctrineProxies"
            ] = DOCTRINE_PROXIES_PATH;
            ...
            foreach ($config["plugins"] as $key => &$plugin) {
                if (is_array($plugin) && isset($plugin["namespace"])) {
                    ...
                    is_string($key) &&
                    is_array($plugin) &&
                    !isset($plugin["namespace"])
                ) {
                    $plugin = ["namespace" => $key, "config" => $plugin];
                } elseif (is_numeric($key) && is_string($plugin)) {
                ...
                foreach ($subfolders as $subfolder) {
                    if (!isset($namespaces[$namespace . "\\" . $subfolder])) {
                        $namespaces[$namespace . "\\" . $subfolder] =
                            $dir . "/" . $subfolder;
                    }
                }
            }
            ...
            foreach ($namespaces as $namespace => $base_dir) {
                if (strpos($class, $namespace) === 0) {
                    $path = str_replace(
                        "\\",
                        "/",
                        str_replace($namespace, $base_dir, $class) . ".php",
                    );
    ...
    {
        $this->config["app.registeredAutoloadCache.lifetime"] =
            (int) $this->config["app.registeredAutoloadCache.lifetime"];
        $this->config["app.assetsUrlCache.lifetime"] =
            (int) $this->config["app.assetsUrlCache.lifetime"];
        $this->config["app.fileUrlCache.lifetime"] =
            (int) $this->config["app.fileUrlCache.lifetime"];
        $this->config["app.eventsCache.lifetime"] =
            (int) $this->config["app.eventsCache.lifetime"];
        $this->config["app.subsiteIdsCache.lifetime"] =
            (int) $this->config["app.subsiteIdsCache.lifetime"];
        $this->config["app.permissionsCache.lifetime"] =
            (int) $this->config["app.permissionsCache.lifetime"];
        $this->config["app.registerCache.lifeTime"] =
            (int) $this->config["app.registerCache.lifeTime"];
        $this->config["app.apiCache.lifetime"] =
            (int) $this->config["app.apiCache.lifetime"];
        $this->config["app.opportunitySummaryCache.lifetime"] =
            (int) $this->config["app.opportunitySummaryCache.lifetime"];

        $this->cache = new Cache($this->config["app.cache"]);
        $this->mscache = new Cache($this->config["app.mscache"]);
        $this->mscache->setNamespace("MS");

        $rcache_adapter = new \Symfony\Component\Cache\Adapter\ArrayAdapter(
            10000,
            false,
            100000,
            10000,
    ...
    protected function _initDoctrine()
    {
        ...
        $doctrine_config->addCustomStringFunction(
            "st_asbinary",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STAsBinary",
        );
        $doctrine_config->addCustomStringFunction(
            "st_astext",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STAsText",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_area",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STArea",
        );
        $doctrine_config->addCustomStringFunction(
            "st_centroid",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STCentroid",
        );
        $doctrine_config->addCustomStringFunction(
            "st_closestpoint",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STClosestPoint",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_contains",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STContains",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_containsproperly",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STContainsProperly",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_covers",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STCovers",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_coveredby",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STCoveredBy",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_crosses",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STCrosses",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_disjoint",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STDisjoint",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_distance",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STDistance",
        );
        $doctrine_config->addCustomStringFunction(
            "st_geomfromtext",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STGeomFromText",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_length",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STLength",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_linecrossingdirection",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STLineCrossingDirection",
        );
        $doctrine_config->addCustomStringFunction(
            "st_startpoint",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STStartPoint",
        );
        $doctrine_config->addCustomStringFunction(
            "st_summary",
            "CrEOF\Spatial\ORM\Query\AST\Functions\PostgreSql\STSummary",
        );

        $doctrine_config->addCustomStringFunction(
            "string_agg",
            "MapasCulturais\DoctrineMappings\Functions\StringAgg",
        );
        $doctrine_config->addCustomStringFunction(
            "unaccent",
            "MapasCulturais\DoctrineMappings\Functions\Unaccent",
        );
        $doctrine_config->addCustomStringFunction(
            "recurring_event_occurrence_for",
            "MapasCulturais\DoctrineMappings\Functions\RecurringEventOcurrenceFor",
        );
        $doctrine_config->addCustomNumericFunction(
            "CAST",
            DoctrineMappings\Functions\Cast::class,
        );
        $doctrine_config->addCustomStringFunction(
            "CAST",
            DoctrineMappings\Functions\Cast::class,
        ...
        $doctrine_config->addCustomNumericFunction(
            "st_dwithin",
            "MapasCulturais\DoctrineMappings\Functions\STDWithin",
        );
        $doctrine_config->addCustomStringFunction(
            "st_envelope",
            "MapasCulturais\DoctrineMappings\Functions\STEnvelope",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_within",
            "MapasCulturais\DoctrineMappings\Functions\STWithin",
        );
        $doctrine_config->addCustomNumericFunction(
            "st_makepoint",
            "MapasCulturais\DoctrineMappings\Functions\STMakePoint",
        );

        // para trabalhar com JSONS
        $doctrine_config->addCustomStringFunction(
            DqlFunctions\JsonbExists::FUNCTION_NAME,
            DqlFunctions\JsonbExists::class,
        );
        $doctrine_config->addCustomStringFunction(
            DqlFunctions\JsonbContains::FUNCTION_NAME,
            DqlFunctions\JsonbContains::class,
        );
    ...
    protected function _initAssetManager()
    {
        if ($this->config["themes.assetManager"] instanceof AssetManager) {
            ...
            $this->assetManager = new AssetManagers\FileSystem(
                $this->config["themes.assetManager"],
            );
        }
    }
    ...
    protected function _initTheme()
    {
        if ($this->subsite) {
            $this->cache->setNamespace(
                $this->config["app.cache.namespace"] . ":" . $this->subsite->id,
            );
    ...
    protected function _initModules()
    {
        ...
        $this->applyHookBoundTo($this, "app.modules.init:before", [
            &$available_modules,
        ]);
        foreach ($available_modules as $module) {
            $module_class_name = "$module\Module";
            $module_config = isset($this->config["module.$module"])
                ? $this->config["module.$module"]
                : [];

            $this->applyHookBoundTo(
                $this,
                "app.module({$module}).init:before",
                [&$module_config],
            );
        ...
        if (!env("DISABLE_PLUGINS")) {
            ...
            foreach ($this->config["plugins"] as $slug => $plugin) {
                ...
                if (class_exists($plugin_class_name)) {
                    $plugin_config =
                        isset($plugin["config"]) && is_array($plugin["config"])
                            ? $plugin["config"]
                            : [];
            ...
            $this->applyHookBoundTo($this, "app.plugins.preInit:after", [
                "plugins" => &$plugins,
            ]);
            ...
            $this->hook("app.modules.init:after", function () use ($plugins) {
                ...
                foreach ($plugins as $plugin) {
                    ...
                    $this->plugins[$slug] = new $plugin_class_name(
                        $plugin_config,
                    );
                }
    ...
    protected function _initStorage()
    {
        ...
        if (
            $storage_class &&
            class_exists($storage_class) &&
            is_subclass_of($storage_class, Storage::class)
        ) {
    ...
    /**
     * Retorna a URL de assets do projeto
     * @return string a url de assets
     */
    public function getAssetUrl()
    {
        return isset($this->config["base.assetUrl"])
            ? $this->config["base.assetUrl"]
            : $this->getBaseUrl() . "assets/";
    }
    ...
    /**
     * Retorna o ID do subsite atual, ou null se o site atual for o site principal
     *
     * @return int|null ID do site atual ou Null para o site principal
     */
    ...
    function getRegisteredGeoDivisions(): array
    {
        $result = [];
        foreach (
            $this->config["app.geoDivisionsHierarchy"]
            as $key => $division
        ) {
            $display = true;
            if (substr($key, 0, 1) == "_") {
            ...
            if (!is_array($division)) {
                // for backward compability version < 4.0, $division is string not a array.
                $d = new \stdClass();
                $d->key = $key;
                $d->name = $division;
    ...
    /**
     * Retorna o array de configuração ou a configuração especificada
     *
    ...
    function lock(
        ...
        if (
            ...
            microtime(true) - filectime($filename) > $expire_in
        ) {
            unlink($filename);
        }
    ...
    /**
     * Cria uma URL para uma ação de controlador
     *
     * @param string $controller_id id do controlador
     * @param string $action_name nome da ação
     * @param array $data dados para passar para a ação
     *
     * @see RoutesManager::createUrl()
     *
     * @return string a URL para a ação
     */
    public function createUrl(
        ...
        return $this->_routesManager->createUrl(
            $controller_id,
            $action_name,
            $data,
        );
     ...
     *
     * Se o nome da classe do repositório não começar com uma barra, esta função irá adicionar \MapasCulturais\Entities\ ao nome da classe
     *
     * @param string $entity_name Nome da classe da entidade
     * @return Repository o Repositório da Entidade
     */
    ...
    function renderMustacheTemplate(
        ...
        if ($this->view->version >= 2) {
            ...
            $template_data->siteDescription = $this->view->dict(
                "site: description",
                false,
            );
        ...
        if (
            !($header_name = $this->view->resolveFileName(
                "templates/" . i::get_locale(),
                "_header.html",
            ))
        ) {
        ...
        if (
            !($file_name = $this->view->resolveFileName(
                "templates/" . i::get_locale(),
                $template_name,
            ))
        ) {
        ...
        if (preg_match_all("#\{\{asset:([^\}]+)\}\}#", $header, $matches)) {
            foreach ($matches[0] as $i => $tag) {
                $header = str_replace(
                    $tag,
                    $this->view->asset($matches[1][$i], false),
                    $header,
                );
            }
        }
        ...
        $this->applyHookBoundTo(
            $this,
            "mustacheTemplate({$template_name}).headerData",
            [&$headerData],
        );
        $_header = $mustache->render($header, $headerData);
        $this->applyHookBoundTo(
            $this,
            "mustacheTemplate({$template_name}).header",
            [&$_header],
        );
        ...
        $this->applyHookBoundTo(
            $this,
            "mustacheTemplate({$template_name}).footerData",
            [&$footerData],
        );
        $_footer = $mustache->render($footer, $footerData);
        $this->applyHookBoundTo(
            $this,
            "mustacheTemplate({$template_name}).footer",
            [&$_footer],
        );
        ...
        $this->applyHookBoundTo(
            $this,
            "mustacheTemplate({$template_name}).templateData",
            [&$template_data],
        );
        $content = $mustache->render($content, $template_data);
        $this->applyHookBoundTo(
            $this,
            "mustacheTemplate({$template_name}).content",
            [&$content],
        );
     ...
     * @param int $status_code código de status HTTP (padrão: 302)
     *
     * @return never
     * @throws Exceptions\Halt
     */
    ...
    /**
     * Trata uploads de arquivos
     *
     * Este método trata uploads de arquivos e retorna uma instância, ou um array de instâncias da Entidade File. O nome do arquivo enviado é higienizado pelo método App::sanitizeFilename
     *
     * Se a chave não existir no array $_FILES, este método retorna null.
     *
     * @param string $key a chave do array $_FILES para tratar
     * @param string $file_class_name nome da classe da entidade de arquivo
     *
            ...
            if (is_array($_FILES[$key]["name"])) {
                ...
                foreach (array_keys($_FILES[$key]["name"]) as $i) {
                    ...
                    foreach (array_keys($_FILES[$key]) as $k) {
                        $tmp_file[$k] =
                            $k == "name"
                                ? $this->sanitizeFilename($_FILES[$key][$k][$i])
                                : $_FILES[$key][$k][$i];
                    }
                ...
                if ($_FILES[$key]["error"]) {
                    throw new Exceptions\FileUploadError(
                        $key,
                        $_FILES[$key]["error"],
                    );
                }
                ...
                $_FILES[$key]["name"] = $this->sanitizeFilename(
                    $_FILES[$key]["name"],
                    $mime,
                );
    ...
    /**
     * Higieniza os nomes dos arquivos enviados substituindo espaços por sublinhados e definindo o nome para minúsculas.
     *
     * Se a chave de configuração 'app.sanitize_filename_function' for invocável, este método a chama após higienizar o nome do arquivo.
     *
     * @param string $filename
     * @param string|bool $mimetype
     *
     * @return string O nome do arquivo higienizado.
     */
     ...
     *
     * Limpa todos os listeners para todos os hooks. Se `$name` for
     ...
     *
     * Retorna um array de hooks registrados. Se `$name` for um nome de
     * hook válido, apenas os listeners anexados a esse hook são retornados.
     * Caso contrário, todos os listeners são retornados como um array associativo cujas
     * chaves são nomes de hooks e cujos valores são arrays de listeners.
     *
     * @param  string     $name     Um nome de hook (Opcional)
     * @return array|null
     */
     ...
     * @param  callable    $callable   Um objeto invocável
     ...
     * Invoca um hook
     * @param  string   $name       O nome do hook
     * @param  array    $hookArg   (Opcional) Argumentos para as funções do hook
     *
     * @return callable[]
     */
    ...
    /**
     * Invoca um hook vinculando callbacks ao objeto alvo
     *
     * @param  object $target_object Objeto para vincular o hook
     * @param  string   $name       O nome do hook
     * @param  array    $hookArg   (Opcional) Argumentos para as funções do hook
     *
     * @return callable[]
     */
    function applyHookBoundTo(
     ...
     */
    public function enqueueOrReplaceJob(
        string $type_slug,
        array $data,
        string $start_string = "now",
        string $interval_string = "",
        int $iterations = 1,
        User|int $user = null,
        Subsite|int $subsite = null,
    ) {
    ...
    public function enqueueJob(
        ...
        try {
            ...
            $this->log->error(
                "ERRO AO SALVAR JOB ($type_slug): " . print_r($e, true),
            );
        }
        return $job;
    }
    ...
    public function executeJob(?string $mock_date = null): int|false
    {
        ...
        if ($job_id) {
            ...
            if ($this->config["app.log.jobs"]) {
                $this->log->debug(
                    "EXECUTING JOB: {$job->id} of type {$job->type}",
                );
     ...
     * @return void
     */
    public function enqueueEntityToPCacheRecreation(
        Entity $entity,
        User $user = null,
    ) {
     ...
     * @return bool
     */
    public function isEntityEnqueuedToPCacheRecreation(
        Entity $entity,
        User $user = null,
    ) {
        $entity_key = $entity->id
    ...
    /**
     * Limpa a lista de entidades com cache de permissão recriado
     *
     * @return void
     */
    ...
    public function recreatePermissionsCache()
    {
        ...
        for ($i = 0; $i < $max_entities; $i++) {
            ...
            try {
                $entity = $this->repo($cache_pending["object_type"])->find(
                    $cache_pending["object_id"],
                );
                if ($entity) {
                    $user_ids = array_map(
                        fn($item) => $item["usr_id"],
            ...
            if ($this->config["app.log.pcache"]) {
                ...
                $this->log->info(
                    "PCACHE RECREATED FOR {$cache_pending["object_type"]}:{$cache_pending["object_id"]} IN {$total_time} seconds\n--------\n",
                );
            }
            $this->_permissionCachePendingQueue = [];
        }
    ...
    function createMailMessage(array $args = []): Email
    {
        ...
        if ($this->config["mailer.alwaysTo"]) {
            foreach ($original as $key => $val) {
                ...
                $message->html(
                    "<strong>ORIGINALMENTE $key:</strong> $val <br>\n $current_body",
                );
            }
        }
    ...
    /**
     * Enfileira um job para envio de e-mail
     *
     * @param Email $message Objeto da mensagem
     *
     * @return void
     */
    ...
    /**
     * Registra o log de envio de e-mail
     *
     * @param Email $message
     ...
     * @return void
     */
    protected function _logMailMessage(
        Email $message,
        ...
        $parseField = function ($item) use (&$parseField) {
            ...
            } elseif (is_object($item) && method_exists($item, "toString")) {
        ...
        $filename =
            date("Y-m") . ($error_message ? "-errors" : "-success") . ".log";
        file_put_contents(
            $folder . $filename,
            $log_line . PHP_EOL,
            FILE_APPEND,
     ...
     */
    function renderMailerTemplate(
        string $template_name,
        array|object $template_data = [],
    ): array {
        if (
            $message = $this->config["mailer.templates"][$template_name] ?? null
        ) {
            $message["body"] = $this->renderMustacheTemplate(
                $message["template"],
                $template_data,
            );
        } else {
    ...
    protected function register()
    {
        ...
        if ($this->view instanceof Themes\BaseV1\Theme) {
            $this->registerController(
                "panel",
                "MapasCulturais\Controllers\Panel",
            );
        }
        ...
        $this->registerController(
            "project",
            "MapasCulturais\Controllers\Project",
        );

        $this->registerController(
            "opportunity",
            "MapasCulturais\Controllers\Opportunity",
        );
        $this->registerController(
            "evaluationMethodConfiguration",
            "MapasCulturais\Controllers\EvaluationMethodConfiguration",
        );

        $this->registerController(
            "subsite",
            "MapasCulturais\Controllers\Subsite",
        );

        $this->registerController(
            "registration",
            "MapasCulturais\Controllers\Registration",
        );
        $this->registerController(
            "registrationFileConfiguration",
            "MapasCulturais\Controllers\RegistrationFileConfiguration",
        );
        $this->registerController(
            "registrationFieldConfiguration",
            "MapasCulturais\Controllers\RegistrationFieldConfiguration",
        );
        ...
        $this->registerController(
            "metalist",
            "MapasCulturais\Controllers\MetaList",
        );
        $this->registerController(
            "eventOccurrence",
            "MapasCulturais\Controllers\EventOccurrence",
        );

        $this->registerController(
            "eventAttendance",
            "MapasCulturais\Controllers\EventAttendance",
        );

        //workflow controllers
        $this->registerController(
            "notification",
            "MapasCulturais\Controllers\Notification",
        );

        // history controller
        $this->registerController(
            "entityRevision",
            "MapasCulturais\Controllers\EntityRevision",
        );
        $this->registerController(
            "permissionCache",
            "MapasCulturais\Controllers\PermissionCache",
        );

        // chat controllers
        $this->registerController(
            "chatThread",
            "MapasCulturais\Controllers\ChatThread",
        );
        $this->registerController(
            "chatMessage",
            "MapasCulturais\Controllers\ChatMessage",
        );

        // registration evaluation
        $this->registerController(
            "registrationEvaluation",
            "MapasCulturais\Controllers\RegistrationEvaluation",
        );
        ...
        foreach ($roles as $role => $cfg) {
            $role_definition = new Definitions\Role(
                $role,
                $cfg->name,
                $cfg->plural,
                $cfg->subsite,
                $cfg->can_user_manage_role,
                $cfg->another_roles,
            );
        ...
        $file_groups = [
            ...
            "avatar" => new Definitions\FileGroup(
                "avatar",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "header" => new Definitions\FileGroup(
                "header",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "gallery" => new Definitions\FileGroup(
                "gallery",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                false,
            ),
            "registrationFileConfiguration" => new Definitions\FileGroup(
                "registrationFileTemplate",
                ["^application/.*"],
                i::__("O arquivo enviado não é um documento válido."),
                true,
            ),
            "rules" => new Definitions\FileGroup(
                "rules",
                ["^application/.*"],
                i::__("O arquivo enviado não é um documento válido."),
                true,
            ),
            "logo" => new Definitions\FileGroup(
                "logo",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "background" => new Definitions\FileGroup(
                "background",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "share" => new Definitions\FileGroup(
                "share",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "institute" => new Definitions\FileGroup(
                "institute",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "favicon" => new Definitions\FileGroup(
                "favicon",
                ['^image/(jpeg|png|x-icon|vnd.microsoft.icon)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "chatImage" => new Definitions\FileGroup(
                "chatImage",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "chatAttachment" => new Definitions\FileGroup(
                "chatAttachment",
                unique: true,
            ),
            "evaluationImage" => new Definitions\FileGroup(
                "evaluationImage",
                ['^image/(jpeg|png)$'],
                i::__("O arquivo enviado não é uma imagem válida."),
                true,
            ),
            "evaluationAttachment" => new Definitions\FileGroup(
                "evaluationAttachment",
                unique: true,
            ),
            "docs-cpf" => new Definitions\FileGroup(
                "docs-cpf",
            ...
            "docs-cnpj" => new Definitions\FileGroup(
                "docs-cnpj",
                ["image/jpeg", "image/jpg", "image/png", "application/pdf"],
                i::__("O arquivo enviado não é um documento válido."),
                true,
            ...
            "docs-cnh" => new Definitions\FileGroup(
                "docs-cnh",
                ["image/jpeg", "image/jpg", "image/png", "application/pdf"],
                i::__("O arquivo enviado não é um documento válido."),
                true,
            ...
            "docs-rg" => new Definitions\FileGroup(
                "docs-rg",
                ["image/jpeg", "image/jpg", "image/png", "application/pdf"],
                i::__("O arquivo enviado não é um documento válido."),
                true,
        ...
        $this->registerFileGroup(
            "registrationFileConfiguration",
            $file_groups["registrationFileConfiguration"],
        );
        ...
        $this->registerFileGroup(
            "registrationEvaluation",
            $file_groups["evaluationImage"],
        );
        $this->registerFileGroup(
            "registrationEvaluation",
            $file_groups["evaluationAttachment"],
        );

        if (
            $theme_image_transformations = $this->view->resolveFilename(
                "",
                "image-transformations.php",
            )
        ) {
            $image_transformations = include $theme_image_transformations;
        } else {
            $image_transformations = include APPLICATION_PATH .
                "/conf/image-transformations.php";
        }
        ...
        if (
            $theme_space_types = $this->view->resolveFilename(
                "",
                "space-types.php",
            )
        ) {
        ...
        $space_meta =
            key_exists("metadata", $space_types) &&
            is_array($space_types["metadata"])
                ? $space_types["metadata"]
                : [];

        if (
            $theme_agent_types = $this->view->resolveFilename(
                "",
                "agent-types.php",
            )
        ) {
        ...
        $agents_meta =
            key_exists("metadata", $agent_types) &&
            is_array($agent_types["metadata"])
                ? $agent_types["metadata"]
                : [];

        if (
            $theme_event_types = $this->view->resolveFilename(
                "",
                "event-types.php",
            )
        ) {
        ...
        $event_meta =
            key_exists("metadata", $event_types) &&
            is_array($event_types["metadata"])
                ? $event_types["metadata"]
                : [];

        if (
            $theme_project_types = $this->view->resolveFilename(
                "",
                "project-types.php",
            )
        ) {
            ...
            $project_types = include APPLICATION_PATH .
                "/conf/project-types.php";
        }
        $projects_meta =
            key_exists("metadata", $project_types) &&
            is_array($project_types["metadata"])
                ? $project_types["metadata"]
                : [];

        if (
            $theme_opportunity_types = $this->view->resolveFilename(
                "",
                "opportunity-types.php",
            )
        ) {
            $opportunity_types = include $theme_opportunity_types;
        } else {
            $opportunity_types = include APPLICATION_PATH .
                "/conf/opportunity-types.php";
        }
        $opportunities_meta =
            key_exists("metadata", $opportunity_types) &&
            is_array($opportunity_types["metadata"])
                ? $opportunity_types["metadata"]
                : [];

        // get types and metadata configurations
        if (
            $theme_subsite_types = $this->view->resolveFilename(
                "",
                "subsite-types.php",
            )
        ) {
            ...
            $subsite_types = include APPLICATION_PATH .
                "/conf/subsite-types.php";
        }
        $subsite_meta =
            key_exists("metadata", $subsite_types) &&
            is_array($subsite_types["metadata"])
                ? $subsite_types["metadata"]
                : [];

        if (
            $theme_seal_types = $this->view->resolveFilename(
                "",
                "seal-types.php",
            )
        ) {
        ...
        $seals_meta =
            key_exists("metadata", $seal_types) &&
            is_array($seal_types["metadata"])
                ? $seal_types["metadata"]
                : [];

        if (
            $theme_notification_types = $this->view->resolveFilename(
                "",
                "notification-types.php",
            )
        ) {
            $notification_types = include $theme_notification_types;
        } else {
            $notification_types = include APPLICATION_PATH .
                "/conf/notification-types.php";
        }
        $notification_meta =
            key_exists("metadata", $notification_types) &&
            is_array($notification_types["metadata"])
                ? $notification_types["metadata"]
                : [];
        ...
        foreach ($space_types["items"] as $group_name => $group_config) {
            ...
            $group = new Definitions\EntityTypeGroup(
                $entity_class,
                $group_name,
                $group_config["range"][0],
                $group_config["range"][1],
            );
            $this->registerEntityTypeGroup($group);

            $group_meta = key_exists("metadata", $group_config)
                ? $group_config["metadata"]
                : [];

            foreach ($group_config["items"] as $type_id => $type_config) {
                $type = new Definitions\EntityType(
                    $entity_class,
                    $type_id,
                    $type_config["name"],
                );
                ...
                $type_meta =
                    key_exists("metadata", $type_config) &&
                    is_array($type_config["metadata"])
                        ? $type_config["metadata"]
                        : [];
                ...
                if (key_exists("metadata", $group_config)) {
                    foreach ($group_meta as $meta_key => $meta_config) {
                        if (
                            !key_exists($meta_key, $type_meta) ||
                            (key_exists($meta_key, $type_meta) &&
                                is_null($type_config["metadata"][$meta_key]))
                        ) {
                ...
                foreach (
                    $type_config["metadata"]
                    as $meta_key => $meta_config
                ) {
                    $metadata = new Definitions\Metadata(
                        $meta_key,
                        $meta_config,
                    );
        ...
        // register taxonomies
        if (
            $theme_taxonomies = $this->view->resolveFilename(
                "",
                "taxonomies.php",
            )
        ) {
        ...
        foreach ($taxonomies as $taxonomy_id => $taxonomy_definition) {
            ...
            $taxonomy_required = key_exists("required", $taxonomy_definition)
                ? $taxonomy_definition["required"]
                : false;
            $taxonomy_description = key_exists(
                "description",
                $taxonomy_definition,
            )
                ? $taxonomy_definition["description"]
                : "";
            $restricted_terms = key_exists(
                "restricted_terms",
                $taxonomy_definition,
            )
                ? $taxonomy_definition["restricted_terms"]
                : false;
            $entities = key_exists("entities", $taxonomy_definition)
                ? $taxonomy_definition["entities"]
                : [];

            $definition = new Definitions\Taxonomy(
                $taxonomy_id,
                $taxonomy_slug,
    ...
    public function registerJobType(Definitions\JobType $definition)
    {
        if (key_exists($definition->slug, $this->_register["job_types"])) {
            throw new \Exception(
                "Job type {$definition->slug} already registered",
            );
        }
     ...
     *
     * @param Definitions\Role $role definição da role
     * @return void
     */
     ...
     * @throws GlobalException
     */
    function registerRegistrationAgentRelation(
        Definitions\RegistrationAgentRelation $def,
    ) {
        $group_name = $def->agentRelationGroupName;
        if (
            $this->_register["registration_agent_relations"][$group_name] ??
            false
        ) {
            throw new \Exception(
                'There is already a RegistrationAgentRelation with agent relation group name "' .
                    $def->agentRelationGroupName .
                    '"',
            );
        }
    ...
    function getRegistrationAgentsDefinitions(): array
    {
        ...
        foreach (
            $this->getRegisteredRegistrationAgentRelations()
            as $groupName => $def
        ) {
     ...
     */
    function getRegisteredRegistrationAgentRelationByAgentRelationGroupName(
        string $group_name,
    ): Definitions\RegistrationAgentRelation|null {
    ...
    function registerChatThreadType(Definitions\ChatThreadType $definition)
    {
        if (isset($this->_register["chat_thread_types"][$definition->slug])) {
            throw new \Exception(
                "Attempting to re-register {$definition->slug}.",
            );
        }
    ...
    /**
     * Registra uma classe de Saída de API
     *
     * Se o $api_output_id não for informado, este método criará o id com base no namespace e no nome da classe
     *
     * @example Exemplo de ids gerados automaticamente: a classe <b>\MapasCulturais\ApiOutputs\Json</b> receberá o id <b>json</b>
     * @example Exemplo de ids gerados automaticamente: a classe <b>\MyPlugin\ApiOutputs\CSV</b> receberá o id <b>myplugin.apiapi_outputs.csv</b>
     *
     * @param string $api_output_class_name nome da classe api_output
     * @param string $api_output_id id da api_output
     *
     */
    public function registerApiOutput(
        $api_output_class_name,
        $api_output_id = null,
    ) {
        if (is_null($api_output_id)) {
            $api_output_id = strtolower(
                str_replace(
                    "\\",
                    ".",
                    str_replace(
                        "MapasCulturais\ApiOutputs\\",
                        "",
                        $api_output_class_name,
                    ),
                ),
        ...
        $this->_register["api_outputs"][
            $api_output_id
        ] = $api_output_class_name;
    }

    /**
     * Retorna a Saída de API pelo nome da classe.
     *
     * Este método retorna null se o nome da classe api_output não estiver registrado ou
     * não for uma subclasse de ApiOutput
     *
     * @param string $api_output_class_name Nome da classe de Saída de API
     *
     * @return ApiOutput|null a Saída de API
     */
    public function getRegisteredApiOutputByClassName(
        $api_output_class_name,
    ): ApiOutput|null {
        if (
            in_array($api_output_class_name, $this->_register["api_outputs"]) &&
            class_exists($api_output_class_name) &&
            is_subclass_of($api_output_class_name, "\MapasCulturais\ApiOutput")
        ) {
    ...
    /**
     * Retorna a Saída de API pelo id da api_output.
     *
     * Este método retorna null se não houver classe api_output registrada sob o id especificado.
     *
     * @param string $api_output_id O Id da Saída de API
     *
     * @return ApiOutput|null A Saída de API
     */
    public function getRegisteredApiOutputById(
        string $api_output_id,
    ): ApiOutput|null {
        $api_output_id = strtolower($api_output_id);
        if (
            key_exists($api_output_id, $this->_register["api_outputs"]) &&
            class_exists($this->_register["api_outputs"][$api_output_id]) &&
            is_subclass_of(
                $this->_register["api_outputs"][$api_output_id],
                "\MapasCulturais\ApiOutput",
            )
        ) {
            $api_output_class_name =
                $this->_register["api_outputs"][$api_output_id];
    ...
    /**
     * Retorna o Id da Saída de API registrada da Saída de API ou nome da classe fornecido.
     *
     * Se o $api_output não for uma Saída de API registrada válida, este método retorna null.
     *
     * @param ApiOutput|string $api_output A Saída de API ou nome da classe
     *
     * @return string|null o id da Saída de API
     */
    ...
    /**
     * Registra uma classe controladora.
     *
     * @param string $id id do controlador.
     * @param string $controller_class_name nome da classe do controlador.
     * @param string $default_action Nome da ação padrão. O padrão é 'index'.
     * @param string $view_dir diretório da view.
     *
     * @throws \Exception
     */
    public function registerController(
        ...
        $this->_register["controllers_view_dirs"][$id] = $view_dir
            ? $view_dir
            : $id;
    }
    ...
    public function getRegisteredControllers(
        bool $return_controller_object = false,
    ): array {
    ...
    /**
     * Retorna o objeto controlador com o id fornecido.
     *
     * Se o controlador estiver registrado, retorna a instância chamando o método i() (singleton getInstance).
     *
    ...
    public function getController(string $controller_id): Controller|null
    {
        ...
        if (
            key_exists($controller_id, $this->_register["controllers"]) &&
            class_exists($this->_register["controllers"][$controller_id])
        ) {
    ...
    /**
     * Retorna o controlador da classe fornecida.
     *
     * Este método verifica se o controlador está registrado antes de tentar obter a instância para retornar.
     *
     * @param string $controller_class O nome da classe do controlador.
     *
     * @return Controller|null O controlador
     */
    public function getControllerByClass(
        ...
        if (
            ...
            return $controller_class::i(
                $this->_register["controllers-by-class"][$controller_class],
            );
        } else {
            return null;
        }
    ...
    /**
     * Retorna o controlador da classe com o mesmo nome da entidade no namespace pai.
     *
     * Se o namespace for omitido no nome da classe, este método assume MapasCulturais\Entities como o namespace da entidade.
     *
     * Este método chama getControllerByClass() para retornar o controlador
     *
     * @param Entity|string $entity O objeto da entidade ou nome da classe
     *
     * @see App::getControllerByClass()
     *
     * @return Controllers\EntityController|null O controlador
     */
    public function getControllerByEntity(
    ...
    /**
     * Retorna o id do controlador da classe com o mesmo nome da entidade no namespace pai.
     *
     * Se o namespace for omitido no nome da classe, este método assume MapasCulturais\Entities como o namespace da entidade.
     *
     * @param Entity|string $entity O objeto da entidade ou nome da classe
    ...
    /**
     * Retorna o id do controlador do objeto controlador ou classe fornecido.
     *
     * @param Controller|string $controller objeto controlador ou nome completo da classe
     *
     * @return string|null
     */
    ...
    /**
     * Retorna o nome da ação padrão do controlador.
     *
     * @param string $controller_id
     *
     * @return string|null
     */
    public function getControllerDefaultAction(
        string $controller_id,
    ): string|null {
        $controller_id = strtolower($controller_id);
        if (
            ...
            return $this->_register["controllers_default_actions"][
                $controller_id
            ];
        } else {
            return null;
    ...
    /**
     * Alias para getControllerDefaultAction.
     *
     * @param string $controller_id
     *
    ...
    /**
     * Registra um Grupo de Tipo de Entidade.
     *
     * @param Definitions\EntityTypeGroup $group O Grupo de Tipo de Entidade a registrar.
     */
    function registerEntityTypeGroup(Definitions\EntityTypeGroup $group)
    {
        if (
            !key_exists(
                $group->entity_class,
                $this->_register["entity_type_groups"],
            )
        ) {
    ...
    /**
     * Retorna o Grupo de Tipo de Entidade da classe da entidade e id do tipo fornecidos.
     *
     * @param Entity|string $entity O objeto da entidade ou nome da classe.
     * @param int $type_id O id do Tipo de Entidade.
     *
     * @return Definitions\EntityTypeGroup|null
     */
    function getRegisteredEntityTypeGroupByTypeId(
        Entity|string $entity,
        int $type_id,
    ): Definitions\EntityTypeGroup|null {
        if (is_object($entity)) {
            $entity = $entity->getClassName();
        }
        ...
        if (key_exists($entity, $this->_register["entity_type_groups"])) {
            foreach (
                $this->_register["entity_type_groups"][$entity]
                as $group
            ) {
    ...
    /**
     * Retorna um array com os Grupos de Tipo de Entidade registrados para o objeto da entidade ou classe fornecido.
     *
     * @param Entity|string $entity O objeto da entidade ou nome da classe
     *
    ...
    {
        if (
            !key_exists($type->entity_class, $this->_register["entity_types"])
        ) {
     ...
     * @throws MappingException
     */
    function getRegisteredEntityTypeById(
        Entity|string $entity,
        int|string|null $type_id,
    ): Definitions\EntityType|null {
        if (is_object($entity)) {
            $entity = $entity->getClassName();
        }
     ...
     */
    function getRegisteredEntityType(
        Entity $entity,
    ): Definitions\EntityType|null {
        return $this->_register["entity_types"][$entity->getClassName()][
            (string) $entity->type
        ] ?? null;
    }
     ...
     * @throws MappingException
     */
    function getRegisteredEntityTypeByTypeName(
        Entity|string $entity,
        string $type_name,
    ): Definitions\EntityType|null {
    ...
    /**
     * Retorna os tipos de entidade registrados para a classe ou objeto da entidade fornecido.
     *
     * @param Entity|string $entity A entidade.
     *
     * @return Definitions\EntityType[]
     */
     ...
     * @return void
     */
    function registerRegistrationFieldType(
        Definitions\RegistrationFieldType $registration_field,
    ) {
        $this->_register["registration_fields"][
            $registration_field->slug
        ] = $registration_field;
    }
     ...
     */
    function getRegisteredRegistrationFieldTypeBySlug(
        string $slug,
    ): Definitions\RegistrationFieldType|null {
    ...
    function registerMetadata(
        ...
        string $entity_class,
        int|string $entity_type_id = null,
    ) {
        ...
        $key = is_null($entity_type_id)
            ? $entity_class
            : $entity_class . ":" . $entity_type_id;
        if (
            !key_exists($key, $this->_register["entity_metadata_definitions"])
        ) {
    ...
    /**
     * Retorna um array com as Definições de Metadados do objeto ou nome da classe da entidade fornecida.
     *
     * Se a classe da entidade fornecida não tiver metadados registrados, retorna um array vazio.
     *
     * @param Entity|string $entity
     * @param int|Definitions\EntityType $type
     *
     * @return Definitions\Metadata[]
     */
    function getRegisteredMetadata(
        Entity|string $entity,
        int|Definitions\EntityType $type = null,
    ) {
        if (is_object($entity)) {
            $entity = $entity->getClassName();
        }
     ...
     */
    function getRegisteredMetadataByMetakey(
        string $metakey,
        Entity|string $entity,
        int $type = null,
    ): Definitions\Metadata|null {
        if (is_object($entity)) {
            $entity = $entity->getClassName();
        }
    ...
    /**
     * Registra uma nova Definição de Grupo de Arquivos para o controlador especificado.
     *
     * @param string $controller_id O id do controlador.
     * @param Definitions\FileGroup $group O grupo a registrar
     */
    function registerFileGroup(
        string $controller_id,
        Definitions\FileGroup $group,
    ) {
        $controller_id = strtolower($controller_id);
    ...
    /**
     * Retorna a Definição de Grupo de Arquivos para o id do controlador e nome do grupo fornecidos.
     *
     * Se a Definição de Grupo de Arquivos não existir, retorna null
     *
     * @param string $controller_id O id do controlador.
     * @param string $group_name O nome do grupo.
     *
     * @return Definitions\FileGroup|null A Definição de Grupo de Arquivos
     */
    function getRegisteredFileGroup(
        string $controller_id,
        string $group_name,
    ): Definitions\FileGroup|null {
    ...
    /**
     * Registra uma nova transformação de imagem.
     *
    ...
    {
        $this->_register["image_transformations"][$name] = trim(
            $transformation,
        );
    }

    /**
     * Retorna a expressão de transformação de imagem.
     *
     * @param string $name o nome de registro da transformação
     *
     * @return string A Expressão de Transformação
     */
    ...
    /**
     * Registra uma nova Definição de Grupo de MetaList para o controlador especificado.
     *
     * @param string $controller_id O id do controlador.
     * @param Definitions\MetaListGroup $group O grupo a registrar
     */
    function registerMetaListGroup(
        string $controller_id,
        Definitions\MetaListGroup $group,
    ) {
    ...
    /**
     * Retorna a Definição de Grupo de MetaList para o id do controlador e nome do grupo fornecidos.
     *
     * Se a Definição de Grupo de MetaList não existir, retorna null
     *
     * @param string $controller_id O id do controlador.
     * @param string $group_name O nome do grupo.
     *
     * @return Definitions\MetaListGroup|null A Definição de Grupo de MetaList
     */
    function getRegisteredMetaListGroup(
        string $controller_id,
        string $group_name,
    ): Definitions\MetaListGroup|null {
    ...
    /**
     * Registra uma Definição de Taxonomia para uma classe de entidade.
     *
     * @param string $entity_class O nome da classe da entidade para registrar.
    ...
    function registerTaxonomy($entity_class, Definitions\Taxonomy $definition)
    {
        if (
            !key_exists(
                $entity_class,
                $this->_register["taxonomies"]["by-entity"],
            )
        ) {
        ...
        $this->_register["taxonomies"]["by-slug"][
            $definition->slug
        ] = $definition;
    }

    /**
     * Retorna a Definição de Taxonomia com o id fornecido.
     *
     * @param int $taxonomy_id O id da taxonomia para retornar
     *
     * @return Definitions\Taxonomy A Definição de Taxonomia
     */
    ...
    /**
     * Retorna a Definição de Taxonomia com o slug fornecido.
     *
     * @param string $taxonomy_slug O slug da taxonomia para retornar
     *
     * @return Definitions\Taxonomy A Definição de Taxonomia
     */
    function getRegisteredTaxonomyBySlug(
        string $taxonomy_slug,
    ): Definitions\Taxonomy|null {
        return $this->_register["taxonomies"]["by-slug"][$taxonomy_slug] ??
            null;
    }

    /**
     * Retorna um array com todas as definições de taxonomia registradas para o objeto ou nome da classe da entidade fornecida.
     *
     * Se não houver taxonomias registradas para a entidade fornecida, retorna um array vazio.
     *
     * @param Entity|string $entity O objeto ou nome da classe da entidade
     *
     * @return Definitions\Taxonomy[] Os objetos de Definição de Taxonomia ou um array vazio
     */
    ...
    /**
     * Retorna a Definição de Taxonomia registrada com o slug fornecido para o objeto ou nome da classe da entidade fornecida.
     *
     * Se a entidade fornecida não tiver o slug de taxonomia fornecido registrado, retorna null.
     *
     * @param Entity|string $entity O objeto ou nome da classe da entidade.
     * @param string $taxonomy_slug O slug da taxonomia.
     *
     * @return Definitions\Taxonomy|null A Definição de Taxonomia.
     */
    function getRegisteredTaxonomy(
        Entity|string $entity,
        string $taxonomy_slug,
    ): Definitions\Taxonomy|null {
        if (is_object($entity)) {
    ...
    /**
     * Retorna as definições dos métodos de avaliação
     *
    ...
    function getRegisteredEvaluationMethods(
        bool $return_internal = false,
    ): array {
        return array_filter($this->_register["evaluation_method"], function (
            Definitions\EvaluationMethod $em,
        ) use ($return_internal) {
     ...
     * @throws GlobalException
     */
    public function registerCountryLocalization(
        CountryLocalizationDefinition $definition,
    ) {
        if (
            key_exists(
                $definition->countryCode,
                $this->_register["country_localizations"],
            )
        ) {
            throw new \Exception(
                "Country localization {$definition->countryCode} already registered",
            );
        }
        $this->_register["country_localizations"][
            $definition->countryCode
     ...
     * @param string $code
     */
    public function getRegisteredCountryLocalizationByCountryCode(
        string $code,
    ): CountryLocalizationDefinition|null {
    ...
    function verifyCaptcha(string $token = "")
    {
        ...
        if (
            !isset($this->config["captcha"]["providers"]) ||
            empty($this->config["captcha"]["providers"])
        ) {
        ...
        if (
            !in_array(
                $provider,
                array_keys($this->config["captcha"]["providers"]),
            )
        ) {
            return false;
        }
        ...
        if (empty($selectedProvider["verify"])) {
            throw new \Exception(
                "No verify url defined for the selected provider",
            );
        }
